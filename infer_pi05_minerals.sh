PYTORCH_ENABLE_MPS_FALLBACK=1 lerobot-record  \
  ...
  --dataset.single_task="Pick the tourmaline and put it in the green container. Pick the basalt and put it in the blue container. Return to rest position." \
  --dataset.num_episodes=1\
  --dataset.episode_time_s=1200 \
  --dataset.push_to_hub=true\
  --policy.path=gimarchetti/pi05_so101_minerals \
  --policy.device=mps \
